...
/* select_for_reads:
	Return: 
		>0 Number of sockets that have become readable,
		0 if timeout,
		<0 if error occurs.
*/
template<class socket_t>
int select_for_reads(int n, socket_t sock[], bool arAvai[], int timeout)
{
	// timeout: in milliseconds
	// sock[]: socket array to wait for
	// arAvai[]: output which sockets are available for read

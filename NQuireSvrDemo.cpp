...
ProcessRecvContent(CString &s, ISendAnswer *pSA)
{
	...
	for (; !s.IsEmpty();)
	{
		...
		if (g_isRecvToStderr)
		{
			...
			log.info("Send to client %s", pWrite);
		}
...
SOCKET udp_listen_sock, Ushort udp_listen_port)
{
	...
	if (re != 0)
	{
		...
		log.error("TCP bind() to port %d fail!", tcp_listen_port);
		return;
	}
	...
	if (re != 0)
	{
		...
		log.error("TCP listen() fail!port = %d", tcp_listen_port);
		return;
	}
	...
	if (re != 0)
	{
		...
		log.error("UDP bind() to port %d fail!", udp_listen_port);
		return;
	}
	...
	log.info("Start listening on UDP port %d.", udp_listen_port);
	log.info("Start listening on TCP port %d.", tcp_listen_port);
	...
	for (;;)
	{
		...
		if (ar_sock_avai[idxTcpListen])
		{
			...
			if (hThread == NULL)
			{
				...
				log.error("UDP Unexpected: Thread creation fail!");
				break;
			}
		...
		else if (ar_sock_avai[idxUdpRead])
		{
			...
			{
				int lastErorr = GetLastError();
				printf("Unexpected: recvfrom() on UDP socket fails with winerr %d.\n", lastErorr);
				log.error("Unexpected: recvfrom() on UDP socket fails with winerr %d", lastErorr);
				continue;
			}
			...
			// serg
			printf("{%s} %s UDP Recv %d bytes from %s\n",
				GetTimeStr(timebuf, sizeof(timebuf)),
				udpbuf,
				bytes_read,
				IPv4Addr2Str(addr_client, str_client));
			log.info("UDP Recv %d bytes from %s", bytes_read, IPv4Addr2Str(addr_client, str_client));
...
main(int argc, char *argv[])
{
	...
	layout->setConversionPattern("%d{%d.%m.%Y %H:%M:%S,%l} [%p] %m%n");
	appender2->setLayout(layout);
	
	log4cpp::Category& sub1 = log4cpp::Category::getInstance(std::string("MAIN"));
	sub1.addAppender(appender2);

	sub1.info("Program started");
	...
	if (tcpport == 0 || tcpport>0xFFff)
	{
		...
		sub1.error("TCP port number %d is invalid", tcpport);
		return 1;
	}
	if (udpport == 0 || udpport>0xFFff)
	{
		printf("UDP port number %d is invalid.\n", udpport);
		sub1.error("UDP port number %d is invalid", udpport);
		return 1;
	}
	...
	if (argc >= 3)
	{
		...
		if (g_nodata_timeout_sec<0)
		{
			...
			sub1.error("no-data-timeout invalid: %d", g_nodata_timeout_sec);
			return 1;
		}
	}
	...
	if (lerr == E_Success)
	{
		...
		sub1.info("bpmap.txt loaded, %d items in product list.", pl.Items());
	}
	else
	{
		if (lerr == E_FileOpenFail)
		{
			...
			sub1.error("Cannot open product list file %s", DEF_BPMAP_FILE);
		}
		else
		{
			printf("Error LoadMapfile.\n");
			sub1.error("Error LoadMapfile");
		}
		return 1;
	}
	s_pl = &pl;
